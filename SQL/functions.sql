...
AS $$ 
     BEGIN
         ...
         RETURN QUERY
         SELECT l.id, l.cod_fiscale, u.nome, u.cognome, u.email, l.categoria, COUNT(p.id) in_corso, l.ritardi
         FROM lettori AS l
         JOIN utenti AS u ON u.id = l.id
         LEFT JOIN prestiti AS p ON p.cod_fiscale = l.cod_fiscale
         WHERE l.id = __id
         GROUP BY l.id, l.cod_fiscale, u.nome, u.cognome, u.email, l.categoria, l.ritardi;
     END;
$$;
...
AS $$
     BEGIN
         ...
         RETURN QUERY
         SELECT l.isbn, l.id, cl.titolo, cl.trama, s.citta, s.indirizzo, l.disponibilita
         FROM libri AS l
...
AS $$
     BEGIN
         ...
         RETURN QUERY
         SELECT cl.isbn, l.id, cl.titolo, cl.trama, s.citta, s.indirizzo, l.disponibilita
...
AS $$
     BEGIN
         ...
         RETURN QUERY
         SELECT cl.isbn, l.id, cl.titolo, cl.trama, s.id, s.citta, s.indirizzo, l.disponibilita, string_agg(CONCAT(a.nome, ' ',a.cognome), ', ')
         FROM cod_libro AS cl
         JOIN libri AS l ON l.isbn = cl.isbn
         JOIN sedi AS s ON s.id = l.sede
         ...
         GROUP BY cl.isbn, l.id, cl.titolo, cl.trama, s.id, s.citta, s.indirizzo, l.disponibilita
         ORDER BY cl.isbn;
     END;
$$;
...
--restituisce le sedi data la città
...
AS $$
     BEGIN
        ...
        RETURN QUERY
        SELECT p.id, p.isbn, p.copia_id, p.cod_fiscale, s.citta, s.indirizzo, p.data_inizio, p.data_fine
        FROM prestiti AS p
        JOIN libri AS l ON l.isbn = p.isbn AND l.id = p.copia_id
        JOIN sedi AS s ON s.id = l.sede 
        
        ORDER BY s.citta, s.indirizzo;
     END;
$$;

--restituisce i prestiti di un dato lettore(il suo codice fiscale)
...
AS $$
     BEGIN
         ...
         RETURN QUERY
         SELECT id, isbn, cod_fiscale, data_inizio, data_fine
         FROM prestiti 
...
AS $$
     BEGIN
         ...
         RETURN QUERY
         SELECT citta, indirizzo, num_copie, num_libri, num_prestiti
...
AS $$
     BEGIN
         ...
         RETURN QUERY
         SELECT citta, indirizzo, isbn, titolo, nome, cognome
         FROM ritardi_vista
         WHERE id = _id;
     END;
$$;
