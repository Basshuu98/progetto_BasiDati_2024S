      ...
      <?php foreach($rows as $row): ?>
        ...
        <tr class="<?= $row["class"] ?>">
          <?php foreach($row["cols"] as $td): ?>
            ...
            <?php if ($td["type"] == "bool"): ?> 
                  <?php if ($td["val"] == "t"): ?>  
                    <td> <span class="icon has-text-success"><i class="fas fa-circle"></i></span> </td>
                  <?php else: ?>
                    <td> <span class="icon has-text-danger"><i class="fas fa-circle"></i></span> </td>
                  <?php endif ?>  
            <?php endif ?>
            ...
            <?php if ($td["type"] == "button"): // form with link td ?>
              <td>
                <form method="POST" action="<?= $td["target"] ?>">
                  ...
                  <?php endforeach ?>
                  <?php if ($td["class"] !== "" && str_contains($td["class"], "disabled")): ?>
                    <button class="button <?= $td["class"] ?> is-small" type="submit" disabled>
                     <?= $td["submit"] ?>
                    </button>
                  <?php else: ?>
